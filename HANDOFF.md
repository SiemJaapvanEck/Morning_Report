# HANDOFF — MOR-3 Tavily citation UI (in review)

> **Last updated:** 21 July 2026 (interactive session with Siem) —
> branch `MOR-3-tavily-citation-ui-2026-07-21`

## Where we stand

`main` has the orchestrator workflow v2 + the krant "A2 · Dagblad +
Verhaallijn" rebuild (merged earlier today, `0ae2df5`). The Linear board is
being seeded for real. First real issue **MOR-3** was built this session and is
awaiting review.
...
1. **MOR-3 review + merge** — awaiting reviewer, then `/merge`. It's `auto-ok`
   but the live "+N" row is only visible after Siem sets the key (below), so the
   visible payoff is deferred, not the merge.
2. **Siem action — set `TAVILY_API_KEY`** in `.env.local` + Vercel. Until then
...
3. **Board seeding** — Siem picked "Something else" for the first substantial
   initiative (not in current docs); he still owes a brain-dump of what it is,
   then `/prd` → `/plan`.
4. **Other Siem-queue items** (from the backlog investigation): localhost-review
