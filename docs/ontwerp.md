...
- **Brandbook + krant "A2 · Dagblad + Verhaallijn" (7 July 2026):** the A3 idle-run krant layout was rejected on review; the krant page is rebuilt to Siem's "Richting A2" design (`Morning Report design/krant-a2-dagblad.html`): per-rubriek rows (articles | sticky impact map + "rubriek in cijfers" | sticky Verhaallijn rail with delen/weken/bronnen stats), lead story with drop cap, thread ribbon and numbered GEVOLGEN. The whole visual system is codified in **`docs/brandbook.md`** (supersedes `docs/design.md`) so future sessions build against documented tokens/recipes instead of re-improvising. Theming moved from 4 fixed themes to a **24-scheme color system on CSS variables** (`app/lib/schemes.ts` generates the CSS + anti-flash script; `mr_thema` migrates to `mr_scheme`; picker in the header). The "rubriek in cijfers" card shows real counts only — the Tavily line appears once Tavily grounding ships.
